...
/*           if (c.clientVars != null) {
  ...
  dynamic handleBinaryOp(var e1, String op, var e2) {
    ...
    r1 ??= (e1 is List<dynamic>)
        ? (((e1[0] == '∀') || (e1[0] == '∃'))
            ? e1
            : resolveDynList(e1, ret: false))
        : (e1 is String)
            ? ((e1[0] == '_') ? vars[e1] ?? e1 : handlePred(e1, null))
            : e1;
    ...
    r2 ??= (e2 is List<dynamic>)
        ? resolveDynList(e2, ret: false)
        : (e2 is String)
            ? ((e2[0] == '_') ? vars[e2] ?? e2 : handlePred(e2, null))
            : e2;
    switch (op) {
      case '∈':
        if (e1 is List<dynamic>) {
        ...
        if ((r1 is String) && (r1[0] == '_')) {
          if ((r2 is List<dynamic>) || (r2 is Set<dynamic>)) {
            if (r2.isEmpty) {
              return false;
            }
            for (int i = 0; i < r2.length; i++) {
              ...
              rvars.addAll(vars);
              rvars[r1] = (r2 is Set<dynamic>) ? r2.elementAt(i) : r2[i];
              varList.add(rvars);
            }
            vars = varList[0];
          ...
          return r2;
        } else if (((r2 is List<dynamic>) || (r2 is Set<dynamic>)) &&
            (r2.isNotEmpty)) {
  ...
  dynamic handleSetOP(List<dynamic> expr) {
    ...
    if (isMap) {
      ...
      if ((e1 == '⊆') || (e1 == '⊂')) {
        if ((e0 is List<dynamic>) && (e2 is List<dynamic>)) {
          for (var m in e0) {
            if (!e2.contains(m)) {
              return false;
            }
          ...
          return true;
        } else if ((e0 is Set<dynamic>) && (e2 is Set<dynamic>)) {
          for (var m in e0) {
            if (!e2.contains(m)) {
              return false;
            }
      ...
      bool isSet = (e0 is Set<dynamic>) || (e2 is Set<dynamic>);
      var e = (e1 == '⊌') ? e0 : (isSet ? <dynamic>{} : <dynamic>[]);
      if ((e1 != '⊌') && (e0 is! String)) {
        e.addAll(e0);
      }
      while (ev.pos <= len) {
        String op = e1;
        var m = ((e2 is List<dynamic>) || (e2 is Set<dynamic>))
            ? e2
            : (isSet ? <dynamic>{} : <dynamic>[]);
        switch (op) {
          //case '⊎':
          case '⊌':
          case '⋃':
          ...
          case '⊖':
            ...
            break;
          case '⥹':
            if (((e0 is List<dynamic>) || (e0 is Set<dynamic>)) &&
                (e2 is int)) {
              int i = e0.length - e2;
              if (i < 0) {
                return null;
